namespace ProjectEuler
{
    class Problem008 : IProblem
    {
        int GetProductOf5DigitsOf(string n, int i)
        {
            return (n[i] - 48) * (n[i + 1] - 48) * (n[i + 2] - 48) * (n[i + 3] - 48) * (n[i + 4] - 48);
        }

        string GetGreatestProductOf5DigitsOf(string n)
        {
            int l = n.Length;
            int currentProd = GetProductOf5DigitsOf(n, 0);
            int maxProd = currentProd;
            int i = 5;
            int maxI = i;
            while (i < n.Length)
            {
                if (n[i] == 48)
                {
                    while (i < l - 5 && GetProductOf5DigitsOf(n, i) == 0)
                        i++;
                    if (i < l - 5)
                    {
                        currentProd = GetProductOf5DigitsOf(n, i);
                        i += 5;
                    }
                    else
                        i++;
                }
                else
                {
                    currentProd = currentProd * (n[i] - 48) / (n[i - 5] - 48);
                    i++;
                }
                if (currentProd > maxProd)
                {
                    maxProd = currentProd;
                    maxI = i - 5;
                }
            }

            return maxProd + " " + n.Substring(maxI, 5);
        }

        public object GetResult()
        {
            string n = "";
            return GetGreatestProductOf5DigitsOf(n);
        }
    }
}
